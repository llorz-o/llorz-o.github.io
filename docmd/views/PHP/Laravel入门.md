...
| HTTP 请求方式 | URL            | 控制器方法 | 路由命名    | 业务逻辑描述                 |
| :------------ | :------------- | :--------- | :---------- | :--------------------------- |
...
定义特殊参数名告知路由解析器根据给定资源 ID 查询模型,并将查询结果作为参数传入.
...
路由绑定默认将传入的`{task}`参数值作为模型主键 ID 进行查询,可通过在模型类中重写`getRouteKeyName()`来实现自定义查询字段.
...
用于访问路由不存在时的处理逻辑,相当于 `*`任意路由(不使用兜底路由默认将返回 404)
...
-   HEAD：与 GET 方法一样，都是向服务器发出指定资源的请求，但是服务器在响应 HEAD 请求时不会回传资源的内容部分（即响应实体），这样我们在不传输全部内容的情况下，就可以获取服务器的响应头信息。HEAD 方法常被用于客户端查看服务器的性能。
...
该请求将被识别为`Route:delete`路由,HTML 表单仅支持`GET/POST`请求
...
1. `\{\{ \}\}` 这种占位符下的输出内容会被`htmlentities`进行 HTML 字符转义(可避免 XSS)
2. `\{\{!! !!\}\}` 不转义直接输出
3. `@` 逻辑控制
...
Laravel 集成了 vue 开发环境

在入口 blade 文件中添加 vue 入口文件
...
此外 Laravel 添加了一些基本的工具,在 axios 中也默认添加了`X-CSRF-TKEN`请求头.

![UTOOLS1567574380715.png](https://user-gold-cdn.xitu.io/2019/9/4/16cfab6e22587ab3?w=898&h=150&f=png&s=25759)

Laravel vue 支持 sass 语法(奇葩的是 laravel 并不支持 vue 组件的全局 scss 导入),你只能通过以下方式导入.
...
表单验证基于`ValidatesRequest`Trait 中的`validate()`方法
...
如果是`ajax`,就处理 422 错误码

如果控制器没有实现`ValidatesRequest`Trait 的话需要使用门面实现验证.
...
-   `clear-compiled`：移除编译过的类文件，比如缓存、Blade 视图文件等
...
或者你可以启用`phpstudy`上的 sql 服务,关闭你的`laradock sql`服务或是修改端口,本质上是`laradock sql`版本较高无法连接.
...
第一个参数表示字段名，第二个参数表示运算符（支持 SQL 所有运算符），第三个参数表示比较值。
...
运行填充`php artisan db:seed --class=PostTagTableSeeder`

在`Post`模型类创建关联查询方法`tags`
...
![UTOOLS1571477896758.png](http://yanxuan.nosdn.127.net/dfdbb1de696377c91732040dcf9025f2.png)
...
创建国家表`countries`,在`users`表中添加`country_id`字段
...
![UTOOLS1571485592295.png](http://yanxuan.nosdn.127.net/cfe139e5cac703be402b97cd1a82485f.png)

在`users`表中添加`country_id`字段值

在模型类`countries`中定义国家与文章的远层一对多关系
...
举例,同时具有文章和视频表,并评论表.评论表同时与文章或视屏具有关联性
...
评论表`comments`中同时存储`item_type`与`item_id`用来区分该条评论所属表
...
php artisan migrate
php artisan make:factory CommentsFactory --model=Comments
...
public function comments(){
    return $this->morphMany('App\Comments','item');
}
```

其中第一个参数是关联模型类名，第二个参数是关联名称，即`$item_id`和`$item_type`中的`$item`部分。当然也可以传递完整参数到`morphMany`方法：

```php
$this->morphMany('App\Comment',$item,$item_type,$item_id,$id);
// 参数$id是posts/videos表的主键。
```

如果需要也可以在`Comment`模型中定义相对的关联关系获取其所属节点：
...
如果`$item`部分不等于`item`可以自定义传入参数到`morphTo()`
...
在`Comments`类中定义`item`方法反向查询评论对应的项
...
这里可能关系图与上面的评论有点像,但是本质上不同.一个评论只能对应文章或视频中的一种,而一个标签可能既对应文章也对应视频

在`tags`中添加`nums`字段

```bash
php artisan make:migration alter_tags_add_nums_table --table=tags
```

添加中间表`taggables`并填充数据

```bash
php artisan make:model Taggable
php artisan make:migration create_taggables_table --create=taggables
php artisan make:factory TaggablesFactory --model=Taggable
php artisan make:seeder TaggablesTableSeeder
php artisan db:seed --class=TaggableTableSeeder
```

最终表结构如下

![UTOOLS1571645425935.png](http://yanxuan.nosdn.127.net/9c37ba4407908b9f1dfb8569c9ad955a.png)

在`Tag`类中编写`posts`和`videos`方法
...
// 完整参数列表如下
$this->morphedByMany('App\Video','taggable','taggables','tag_id','taggable_id');
```

在`Post`和`Video`表中添加方法`tags`
...
// 完整参数列表如下
$this->morphToMany('App\Tag','taggable','taggables','taggable_id','tag_id',false);
```

第三个参数是对应关系表名，最后一个值若为`true`，则查询的是关联对象本身，若为`false`，查询的是关联对象与父模型的对应关系。
...
将`database`文件中`sql`配置项的`strict`严格改为 false
